...
typedef struct memData {
	int pID;
	DWORD memory;
	int value_wanted_int;
	double value_wanted_double;
	int bytes_to_read;
	int datatype;
	int searchtype; //1 = exact value, 2 = increased value;
} memdat;

typedef struct memGet {
	DWORD memory;
	int value_returned_int;
	double value_returned_double;
} memget;
...
USHORT To_Scancode(CHAR code, BOOLEAN *caps);
USHORT To_Special(CHAR code);
